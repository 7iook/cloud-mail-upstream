const zh = {
    ...
    codeRecognitionMode: '识别方式',
    codeRecognitionOff: '关闭',
    codeRecognitionRuleOnly: '仅规则识别',
    codeRecognitionRuleAi: '规则识别 + AI 补位',
    codeRecognitionModeDesc: '规则识别不消耗 AI 额度；AI 补位仅在规则没把握时调用 Workers AI。',
    oss: '对象存储',
    osDomain: '访问域名',
    ...
    shareSecretOnce: '请立即复制并妥善保存这条完整链接。日后也可以在分享详情里重新查看它。',
    ...
    shareVisitLink: '验证链接',
    shareVisitLinkOpen: '打开',
    shareVisitNothingFound: '未识别到验证码 / 验证链接',
    shareVisitCopyAll: '复制邮件全文',
    shareVisitCopiedAll: '已复制全文',
    shareVisitNewMailToast: '收到新邮件',
    shareVisitFrom: '来自',
    ...
    shareDomainNotConfigured: '这个域名未在本站配置，无法为它创建分享。请检查拼写，或改用本站已配置的域名。',
    shareEmailRejected: '有地址不符合本站的邮箱规则（长度或保留前缀），请调整后重试。',
    shareEmailUnavailable: '有邮箱当前不可用（可能属于其他用户或超出数量配额）。',
    ...
    shareRenewLabel: '有效期',
    shareRenewKeep: '保持不变',
    shareRenewBy: '延长 {duration}',
    shareRenewExact: '指定失效时间',
    shareRenewTarget: '保存后失效于：{time}',
    shareRenewCeiling: '最多可延长到 {time}（自创建起 {days} 天）。',
    shareRenewInvalid: '请填写一个有效的失效时间。',
    shareRenewPast: '新的失效时间必须晚于现在。要让它立刻失效，请用“销毁”。',
    shareRenewTooLong: '超出上限：自创建起最多 {days} 天，最晚只能延到 {time}。',
    shareCreateFailed: '创建分享失败，请稍后重试。',
    shareUpdateConflict: '这条分享刚被改动过（可能是另一位管理员保存了修改，或重置了访问密钥）。请刷新后重试。',
    shareUpdateFailed: '保存失败，请稍后重试。',
    ...
    shareLinkTitle: '分享链接',
    shareLinkRegenerate: '重新生成链接',
    shareLinkHint: '想确认当初发出去的是哪一条，点「查看链接」原样取回；若链接可能外泄或已经丢失，请重新生成一条。',
    shareLinkReveal: '查看链接',
    shareLinkRevealed: '这是当初创建时发出的那条链接，原样取回。',
    shareLinkRevealHide: '收起',
    shareSecAbsent: '这条分享创建于本功能上线之前，没有留下可取回的凭据。如果链接已经找不到了，请重新生成一条。',
    shareSecUnavailable: '服务端未配置凭据密钥，暂时无法取回链接。请联系管理员或运维处理——这不是你的操作问题，重试也不会有帮助。',
    shareSecKeyRetired: '加密密钥已经轮换退环，这条分享的链接无法再取回。请重新生成一条。',
    shareSecCorrupted: '这条分享的凭据数据异常，无法取回链接。请联系管理员排查。',
    shareLinkRevealFailed: '无法取回链接：分享状态可能刚刚变化。请刷新后重试；若仍然失败，请联系管理员。',
    shareLinkRegenerateConfirm: '旧链接会立刻作废，正在查看的访客会当场断开，需要你把新链接发给他们。有效期与可见邮件范围保持不变。',
    shareLinkOnce: '旧链接已作废。请立即复制并妥善保存这条新链接，日后也可以在分享详情里重新查看它。',
    shareLinkSaved: '我已保存',
    shareLinkRegenerated: '已生成新链接',
    shareLinkRegenerateFailed: '无法重新生成链接：分享状态可能刚刚变化。请刷新后重试。',
    shareDetail: '详情',
