...
|---|---|---|---|
| `POST /mailShare/create` | 扩展(2026-08-26 增 `emails[]`) | `{emails?: string[]`(完整地址,share-fullchain P2)` \| accountIds: number[]`(或旧 `accountId` 单值,兼容 AC-CAP-10)`, durationSeconds, name?, remark?, maxSessions?, messageLimit?, onlyMessagesAfterCreated?, otpExtractionEnabled?, autoRefresh?, refreshIntervalMs?, showFullAddress?, authKeyEnabled?}` + `Idempotency-Key` 头。**emails 语义**:非空即优先,忽略同请求 `accountIds`;trim+小写+去重;逐地址 find-or-create 建号(建号不变量唯一居所 `mailbox-provision.js`,零写入预校验,任一地址不过整单拒绝);`emails.length===1` **不过** V2 栅栏;`emails.length>1` 且 V2=true → 一条 multi(AC-LIFE-11 语义);`emails.length>1` 且 V2=false → **同一请求 N 条单分享(批量分流,DC-P0-1),禁止用栅栏吞掉批量**;`emails` 数量 > `SHARE_BINDING_LIMIT`(50)→ `SHARE_BINDING_LIMIT_EXCEEDED`。格式/前缀不过 → `SHARE_EMAIL_INVALID`;域名未配置 → `SHARE_DOMAIN_NOT_CONFIGURED`;账号侧不可用(已删/他人/配额/角色域名权限)→ `SHARE_ACCOUNT_FORBIDDEN`(与 accountId 路径同码,不扩大可探测面)。旧语义:`accountIds.length > 1`、`authKeyEnabled=true`、有限 `maxSessions` 均需 `SHARE_CAPABILITY_V2=true`,否则 `SHARE_CAPABILITY_NOT_ENABLED`(全能力栅栏,AC-LIFE-10/11);`accountIds` 数量 > `SHARE_BINDING_LIMIT`(50)→ `SHARE_BINDING_LIMIT_EXCEEDED`(AC-CAP-13)。**Deprecated(行为已删除)**:向导「选已注册账号下拉 / 前缀+域名拼接」作为创建分享前置;设置页「添加账号」拼接 UI 保留,底层同走 `mailbox-provision.js` | 单地址与 V2 multi:`shareId, lid, shareUrl, sec`(仅首次)`, authKey`(仅首次且启用时)`, shareType, bindings[], mailbox`(emails 路径);V2=false 批量:`{shares: [...]}`(每条各含上述字段);重放含 `idempotentReplay: true` 且无 `sec`/`authKey`(批量重放为 `{shares, idempotentReplay}`,靠幂等行 `response_fingerprint` 的 lid 清单回读;响应丢失恢复流程见 AC-CAP-14);同时双写主表 `account_id` = 主 Binding(R2-A1) |
...
| create `emails[]` 含格式/前缀不合规地址 | mailbox-provision → mail-share-service | `SHARE_EMAIL_INVALID`(新码,share-fullchain P2;零写入整单拒绝) |
| create `emails[]` 含未配置域名 | mailbox-provision → mail-share-service | `SHARE_DOMAIN_NOT_CONFIGURED`(新码,share-fullchain P2;前端出「域名未配置」友好提示) |
| create `emails[]` 含账号侧不可用地址(已删/他人/配额/角色域名权限) | mailbox-provision → mail-share-service | `SHARE_ACCOUNT_FORBIDDEN`(沿用同码,不为新入口扩大可探测面) |
...
- 2026-08-26 · executor(share-fullchain 整改执行者,决策卡 `.agent-workspace/.archive/2026-08-26/share-link-fullchain/share-fullchain-decision-card.md`):
  - **P2 · create `emails[]` + 批量分流(DC-P0-1 改方向)**:API 契约表 create 行扩展(完整地址 find-or-create、`emails.length===1` 不过 V2 栅栏、`emails.length>1` 且 V2=false → 同一 D1 batch 出 N 条单分享 `{shares:[...]}`,禁止多 Binding);建号不变量唯一居所下沉 `mail-worker/src/service/mailbox-provision.js`(DC-P0-2),`account-service.add` 改为调用它(addEmail 开关/Turnstile 仍只包装设置页入口);新错误码 `SHARE_EMAIL_INVALID` / `SHARE_DOMAIN_NOT_CONFIGURED`(域名校验用 `configuredDomains(c)` 精确匹配,禁子串);向导「选已注册账号下拉 / 前缀+域名拼接」作为创建分享前置标注 **Deprecated(行为已删除)**,`accountId`/`accountIds` 仍留给「分享当前邮箱」快捷入口(AC-CAP-10)。
  - **P4 · gone 收缩波及本 spec**:AC-AUTH-02 / AC-EDGE-03 / AC-SEC-07 已行内修订——`lid` 不存在与 `REVOKED`(gone)自本日起答浏览器原生 HTTP 404 空 body,不再落入 `SHARE_UNAVAILABLE` 不可区分族;完整口径与入口清单见 `docs/specs/mail-share/design.md` 同日 Update Log。
